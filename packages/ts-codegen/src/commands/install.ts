...
export default async (argv: MinimistArgs) => {
  ...
  const packageInfo = JSON.parse(
    readFileSync('./smart-contracts/package.json', 'utf-8')
  );
  ...
  thisPackage.devDependencies = Object.fromEntries(
    Object.entries(thisPackage.devDependencies).sort()
  );

  writeFileSync(
    join(cur, 'package.json'),
    JSON.stringify(thisPackage, null, 2)
  );
