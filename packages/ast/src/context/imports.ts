...
export type GetUtilFn = <TContext = RenderContext>(
  ...args: any[]
) => (context: TContext) => ImportObj;
...
export const convertUtil = (
  ...
  if (!Object.prototype.hasOwnProperty.call(registeredUtils, util)) return null;
...
export const getImportStatements = (
  ...
  const imports = modifiedImports.reduce(
    (m: Record<string, ImportObj[]>, obj) => {
      m[obj.path] = m[obj.path] || [];
      const exists = m[obj.path].find(
        (el: ImportObj) =>
