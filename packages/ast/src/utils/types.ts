...
export const getTypeOrRef = (obj: JSONSchema): t.TSType => {
  if (obj.$ref) {
    ...
    throw new Error(
      'Type definition not supported or missing: ' + JSON.stringify(obj)
    );
  }
};
...
export const detectType = (type: string | string[]) => {
  ...
  if (Array.isArray(type)) {
    if (type.length !== 2) {
      throw new Error(
        '[getType(array length)] case not handled by transpiler. contact maintainers.'
      );
    }
    const [nullableType, nullType] = type;
    if (nullType !== 'null') {
      throw new Error(
        '[getType(null)] case not handled by transpiler. contact maintainers.'
      );
    }
...
export const getTypeInfo = (context: RenderContext, info: JSONSchema) => {
  let type = undefined;
  ...
  if (Array.isArray(info.anyOf)) {
    ...
    if (nullType?.type !== 'null') {
      throw new Error(
        '[nullableType.type]: case not handled by transpiler. contact maintainers.'
      );
    }
    if (!nullableType?.$ref) {
      if (nullableType.title) {
        ...
        throw new Error(
          '[nullableType.title] case not handled by transpiler. contact maintainers.'
        );
      }
    } else {
    ...
    if (info.type === 'array') {
      if (typeof info.items === 'object' && !Array.isArray(info.items)) {
        if (info.items.$ref) {
          ...
          type = t.tsArrayType(
            t.tsTypeReference(t.identifier(info.items.title))
          );
    ...
    if (info.type.length !== 2) {
      throw new Error(
        'please report this to maintainers (field type): ' +
  ...
  if (Array.isArray(jsonschema.properties[prop].allOf)) {
    ...
    if (uniqUnionTypes.length === 1) {
      return propertySignature(
        camelize ? camel(prop) : prop,
        t.tsTypeAnnotation(
          t.tsTypeReference(t.identifier(uniqUnionTypes[0] as string))
        ),
        isOptional
      );
    } else {
        ...
        t.tsTypeAnnotation(
          t.tsUnionType(
            uniqUnionTypes.map((typ) =>
              t.tsTypeReference(t.identifier(typ as string))
            )
          )
        ),
        isOptional
...
export const getParamsTypeAnnotation = (
  ...
  const typedParams = keys.map((prop) =>
    getPropertySignatureFromProp(context, jsonschema, prop, camelize)
  );
